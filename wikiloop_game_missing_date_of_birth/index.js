...
async function getWikidataClaims(qNumber) {
	...
	await rp(options).then(res =>{
		if (res.claims && res.claims.hasOwnProperty('P569')) {
			for (let p of res.claims.P569) {
				let claim = {};
				claim.id = p.id;
...
function generateYesEntry(missingValue, referenceSnaks, qNumber){
	...
	apiAction.summary = 'Distributed game missing date of birth. Update P569.';
	let data = {};
	data.claims = [];
	let claim = {};
	...
	claim.mainsnak = {
		...
		datavalue: {
			...
			// TODO: decide calendar model by entity
...
app.use(async (ctx, next) => {
  ...
  if (q.action == 'desc') {
  	out.label = {en: 'Missing Date of Birth'};
  	out.description = {en: "Import missing date of birth from wikipedia to wikidata."}
  	out.instructions = {en: "*Click \"Accept\" to add a date of birth claim(P569) to the wikidata entity, meanwhile set the wikipedia pages as references.\n" +
  	"*Click \"Reject\" to refuse the suggestion.\n*If you're not sure, click \"I don't know\".\n" +
  	"*The suggested date of birth comes from wikipedia articles. There might be error due to outdated wikipedia page snapshots, program parsing error or wrong wikipedia info." +
  	...
  	out.icon = 'https://upload.wikimedia.org/wikipedia/commons/4/42/Nuvola_cake_4.png';
  	ctx.jsonp(out);
...
app.use(async ctx => {
	...
	while(out.tiles.length < num) {
		...
		while (out.tiles.length < num && i < rows.length) {
			...
			// Skip the entities that already have date of birth record
			if (claims.length >= 1) {
				continue;
			}
